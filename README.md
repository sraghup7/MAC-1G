...
**Status: Stage 3 complete.** The reference model, stimulus generator, testbenches
and every quality gate exist and run. There is no RTL yet: `rtl/gem_mac_stub.v` is
a port-only stub, so the regression fails on purpose and the two harness
self-tests are the only green runs. That is the intended state — the thing that
decides whether the hardware is correct was built before the hardware.
...
That is model tests → committed-vector staleness → Verilator lint → the scenario
regression, in that order because it is the order that makes a failure
diagnosable. It exits nonzero today, because there is no design for the
regression to pass against.
...
rtl/          the design (a stub today) and the shared parameter header
...
That habit is not decoration. Reviewing this stage found a skewed comparison, a
