...
and the fix was to R6, not to the RTL: refuse the octet that would be the 1501st
before sending it, so the wire carries a 1517-octet frame marked bad rather than
an oversize one. See [B.4d](spec/PROJECT_SPEC.md) and V-16. Its
`tx_expected.hex` is the only vector datum in the repository that changed; the
other 47 files regenerated byte-identically.

Measured, not estimated: **993 LUTs, 1403 FFs, no BRAM, no DSP** (4.8% of the
device, against a B.2 budget of 2000 LUTs), **+2.135 ns** slack at 125 MHz out of
