...
|---|---|---|---|
| **R14 / R20 / V-2** | RGMII I/O timing margin is thin on both directions and neither can be confirmed by simulation: TX closes at **+58 ps** worst case, and the five RX input-delay checks are signed off by derivation under a fenced waiver (Vivado's ZHOLD modeling artifact, task 4e) rather than by a passing STA number. **The fallback this item names is no longer available as written** — see the correction below. | Bring-up step 5, a scope or the `make debug` ILA on `GTX_CLK`/`TXD0` and the RX side. The fallback previously named here (a KSZ9031RNX `GTX_CLK` pad-skew register, over MDIO) does not exist on the physical chip; see the JL2121(D) correction below for what the real escalation path is. | README's timing section · `Documents/RGMII I-O Timing Derivation.md` · `docs/reports/stage6-part2/task-4e-report.md` · `verification_plan.md` rows V-2, R14, R20 |
...
| **A.2 (B.5): JL2121(D), not KSZ9031RNX — RGMII timing budget unconfirmed against the real chip** | B.5 step 3 read `phyid=0x937c4032`, which the JLSemi JL2121(D) datasheet confirms with certainty (PHYIDR1 default `0x937c`, PHYIDR2 `0x402x` with the revision in the low nibble). The board's PHY is neither the RTL8211 the spec first assumed nor the KSZ9031RNX a mis-extracted manual quote claimed. Three consequences, all now closed except the last: `rtl/gem_mdio.v`'s register-0x1F speed/duplex read was decoding the wrong register — **fixed**, it now pages to the PHY Specific Status Register correctly; `PHY_ADDR` defaulted to a guessed 0 instead of the strapped 1 — **fixed**, confirmed and corrected from the real manual's Table 8-1; and R14/R20's MDIO pad-skew fallback (KSZ9031RNX MMD `2h`/register `8h`) **does not exist** on the JL2121(D) — RX/TX clock delay is set by hardware strap pins (`RXDLY`/`TXDLY`), both confirmed populated to add 2 ns (same table) — **the strap states are known, but the RGMII AC timing budget built from them is not yet re-derived** from the JL2121(D)'s own datasheet (`DS009-JL2121(D)-v1.09-Preliminary`, Chapter 4.7). | Re-derive the RGMII AC timing budget from the JL2121(D) datasheet's Chapter 4.7 using the confirmed 2 ns RXDLY/TXDLY strap delays, before trusting R14/R20's margin numbers as chip-accurate (the 58 ps and derivation-based RX sign-off may still hold, since both chips target the same RGMII v2.0 windows, but that is now an assumption to check, not a fact in hand). No schematic pull needed any more — the real manual (`Manuals/AX7035B_UG.pdf`) already gives the strap states. | `spec/PROJECT_SPEC.md` A.2's B.5 correction · `rtl/gem_mdio.v` header note · `Manuals/AX7035B_pinout_notes.md` · `Documents/RGMII I-O Timing Derivation.md` · `Documents/RX Clock Deskew Design.md` |
