...
| **PHY reset hold time (`tSR`)** | `gem_clk_rst` holds `phy_rst_n` low for ≥10 ms, sourced to the KSZ9031RNX datasheet (A.2's original assumption). The real chip is a JL2121(D) (A.2's B.5 correction); its own reset-timing figure (datasheet §4.7.1) has not been checked against this value. Not necessarily wrong — 10 ms is a generous margin, not a tight one — but sourced to the wrong datasheet until checked. | Read the JL2121(D) datasheet's own power-up/reset timing table and confirm 10 ms still clears it; cheap to check without a rebuild if it doesn't (the counter is a parameter). | `rtl/gem_clk_rst.v` · `spec/PROJECT_SPEC.md` A.2's B.5 correction |
