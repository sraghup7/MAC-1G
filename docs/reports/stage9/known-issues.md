...
| **A.2 (B.5): JL2121(D), not KSZ9031RNX — RGMII timing budget unconfirmed against the real chip** | B.5 step 3 read `phyid=0x937c4032`, which the JLSemi JL2121(D) datasheet confirms with certainty (PHYIDR1 default `0x937c`, PHYIDR2 `0x402x` with the revision in the low nibble). The board's PHY is neither the RTL8211 the spec first assumed nor the KSZ9031RNX the ALINX manual claimed. Two concrete consequences: `rtl/gem_mdio.v`'s register-0x1F speed/duplex read was decoding the wrong register (0x1F is a page-select register on this chip, not a speed encoding) — **fixed**, it now pages to the PHY Specific Status Register correctly; and R14/R20's MDIO pad-skew fallback (KSZ9031RNX MMD `2h`/register `8h`) **does not exist** on the JL2121(D) — RX/TX clock delay is set by hardware strap pins (`RXDLY`/`TXDLY`), fixed at board population, not MDIO-programmable — **not fixed, and cannot be from software**. The RGMII timing budget itself (B.1b's 1.2 ns RX default delay, the `TsetupR`/`TholdR`/`TsetupT`/`TholdT` window, PHY reset `tSR`) is KSZ9031RNX-sourced and not yet re-derived from the JL2121(D)'s own datasheet (`DS009-JL2121(D)-v1.09-Preliminary`, Chapter 4.7). | Re-derive the RGMII AC timing budget from the JL2121(D) datasheet before trusting R14/R20's margin numbers as chip-accurate (the 58 ps and derivation-based RX sign-off may still hold, since both chips target the same RGMII v2.0 windows, but that is now an assumption to check, not a fact in hand); and pull the AX7035B schematic to read the `RXDLY`/`TXDLY` strap states before relying on any assumed RX/TX clock delay value. | `spec/PROJECT_SPEC.md` A.2's B.5 correction · `rtl/gem_mdio.v` header note · `Documents/RGMII I-O Timing Derivation.md` · `Documents/RX Clock Deskew Design.md` |
