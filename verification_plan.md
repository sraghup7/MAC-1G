...
| **V-24** | ~~RGMII RX input timing cannot close on the deskewed-BUFG clocking, structurally~~ | **Closed -- the residue is Vivado's ZHOLD model, and the real defect under it is fixed.** Task 4e measured the routed feedback path directly (0.936/1.974 ns fast/slow) against the compensation arc the tool applies (-2.703/-6.062): the STA capture-clock arrival is a constructed constant (`IBUF+ccio + arc + fwd` invariant across the BUFG and BUFH builds), injecting ~2.3 ns of phantom spread onto exactly the five RX input-delay checks. Manual generated-clock re-declaration leaves the arc intact; `COMPENSATION=EXTERNAL` is rejected for on-chip loops ([Timing 38-290]). Underneath the artifact sat a genuine physical miss -- the input-side IBUF spread passes straight through a deskew loop, so at 0 degrees the slow-corner hold failed by ~0.33 ns. Fixed with `CLKOUT0_PHASE = -45 deg` (-1000 ps, centred in the feasible window): predicted physical margins +0.68/+1.13 setup, +1.12/+0.67 hold. Gate 2 now waives exactly those five endpoints under a fenced derivation (count asserted, -5.000 ns envelope) and signs R20's RX half off by derivation plus bench. | Full evidence: docs/reports/stage6-part2/task-4e-report.md. Outstanding either way: the REF_JITTER1 = 0.125 second run of criterion A; bench confirmation of the derived margins |
| **V-25** | An RX AXI-S frame in flight during a link event aborts without tlast | Consequence of the Stage 6 part 2 reset architecture (`rx_path_rst_n` resets `gem_rx_egress`, so a link drop cannot splice stale FIFO contents into a new frame as well-formed output). Owner decision (a): accept the mid-frame tvalid drop for v1, documented in B.4a's amendment. | Demonstrated behaviour must be pinned by a testbench scenario (criterion D8); before then it stays documented rather than latent. The clean in-band abort (final beat with tlast=1, tuser=1) is scheduled as its own task before anything real consumes the RX port. Also outstanding from the task-4d/4e deferrals: criterion D6/D7 (FIFO integrity across link drop, phantom statistics) and the REF_JITTER1 = 0.125 second run of criterion A. |
