...
| R14 | Documented clock/data skew mechanism | `gem_mmcm` asks the MMCM for a −73.125° (1.625 ns) second output, the nearest achievable step to B.1b's −72° (1.6 ns) intent; `tb_gem_clk_rst` confirms both outputs exist and start together | bench + static timing | **open** · see **V-2** — the mechanism is now implemented rather than only specified, but a phase shift is an I/O timing property and simulation passes at any phase. Achievable step: 45°/8 = 5.625°, so −72° rounds to −73.125° (1.625 ns), still 0.375 ns inside the datasheet window. `write_bitstream`'s DRC (AVAL-139) refuses a `CLKOUT1_PHASE` that is not an exact multiple of the step, which is how a stale −72.000 literal in `rtl/gem_mmcm.v` was found and corrected — nothing had run `write_bitstream` against `gem_top` before Stage 6 retargeted the build onto it |
