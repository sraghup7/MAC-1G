...
| **V-25** | An RX AXI-S frame in flight during a link event aborts without tlast — **accepted for v1, and now demonstrated rather than latent (D8). Stays open because the behaviour stands, not because it is unproven.** | Consequence of the Stage 6 part 2 reset architecture (`rx_path_rst_n` resets `gem_rx_egress`, so a link drop cannot splice stale FIFO contents into a new frame as well-formed output). Owner decision (a): accept the mid-frame tvalid drop for v1, documented in B.4a's amendment. | **D8 is done:** `tb_gem_top` section 8 drops the link with a frame genuinely in flight *on the AXI-S port* (not merely on the pins — the RX pipeline is 13 cycles deep, so those are different instants) and asserts the frame ends with no terminating beat. The check arms its own `tlast` watcher against ordinary traffic first and requires it to latch, because a test that passes on the *absence* of something is exactly the test a broken watcher also passes. The clean in-band abort (final beat with tlast=1, tuser=1) is still scheduled as its own task before anything real consumes the RX port — and when it lands, section 8 is written to fail deliberately rather than silently accept both behaviours. **Criteria D6 and D7 are now closed** — `tb_gem_top` sections 6 and 7 drop the recovered clock (a link event, not a board reset: the one case in which the FIFO's two halves can reset asymmetrically) and assert zero octets reach the AXI-S port and zero counters move. Both were demonstrated to fail before being trusted: D6 reports **491 fabricated octets** once *both* protections are removed (`gem_mac`'s `rx_path_rst_n` wiring **and** `gem_rx_fifo`'s internal `rd_rst_eff_n` cross-coupling — either alone is sufficient to prevent the corruption, which is why defeating one is not enough to prove the gate), and D7 reports `rx_ok 25→26` once the five `dst_rst_n` connections revert to `tx_rst_n`. D7 additionally drives one extra frame before the drop so the event toggles sit at **odd** parity: `toggle` resets to 0, so a phantom edge exists only after an odd number of events, and dropping on an even count passes against broken RTL. The REF_JITTER1 = 0.125 second run of criterion A is also complete (`docs/reports/stage7/task-report-refjitter.md`). |
