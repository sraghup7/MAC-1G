...
Document status: v1.3 — Stage 4 complete, Stage 5 under way (the clock/reset block
and R17's UART readout are built and tested; V-20 is closed, and V-21 is what the
stage is now waiting on). Versioned alongside the RTL.
...
| **V-21** | The board pins for the reset key and the UART are documented only in schematic figures | `gem_clk_rst` takes `ext_rst_n` from the board's reset key and R17's readout needs a TXD pin, and neither is in the ALINX manual's text: Part 20 (keys) and Part 13 (USB to serial port) describe the hardware in prose and put the numbers in figures ManualsLib's text layer does not carry. Re-checked page by page, so the gap is the document's, not the reading of it. | **Pins found, not yet confirmed.** ALINX's own demo projects carry them, mirrored at `github.com/BLANK2077/AX7035`: reset key **F20**, `uart_rx` **G15**, `uart_tx` **G16** (FPGA-side naming, so this design transmits on G16), keys M13/K14/K13/L13. Believable because the same files carry `sys_clk` Y18 and the four LED pins this project already had from the manual and all five match, because all twelve verify as real package pins in the `xc7a35tifgg484-1L` database with G15/G16 an L2P/L2N pair, and because their `LVCMOS33` corroborates the bank-16 voltage `constrs/pins.xdc` had assumed. Enough to write the XDC against and flag, in the way that file already flags the bank voltage - not enough to call closed. It closes against `SCH/SCH.pdf` or the board itself, and the first thing to try if the serial port is silent at bring-up is swapping G16 for G15. Details and provenance in `Manuals/AX7035B_pinout_notes.md`. |
