...
| R14 | Documented clock/data skew mechanism | `gem_mmcm` asks the MMCM for a −55.000° (1.2222 ns) second output, on a 1125 MHz VCO with both dividers at 9; `tb_gem_clk_rst` confirms both outputs exist and start together, and post-route `report_timing` against `constrs/rgmii_timing.xdc` measures what the pins actually get | bench + static timing | **green** (static timing) — the static half is closed. Worst-case TX setup margin is **+0.058 ns** on `rgmii_txd[0]`, the other four ports at +0.065 / +0.072 / +0.068 / +0.068 ns, and hold ≥ +1.645 ns everywhere; measured on the committed tree, not inherited from the diagnostic builds that found the value. That is a real pass on a check that was violated by −0.351 ns before Stage 6 part 2 — and it is thin, not comfortable, which this row states rather than rounds up. Getting there needed the MMCM's VCO restructured, not its phase alone: B.1b's −72° was never achievable on the old 45/8 = 5.625° grid (`write_bitstream`'s AVAL-139 DRC refuses an off-grid `CLKOUT1_PHASE`, which is how a stale −72.000 literal in `rtl/gem_mmcm.v` was found — nothing had run `write_bitstream` against `gem_top` before Stage 6 retargeted the build onto it), −73.125° was achievable and violated, and moving the VCO to 1125 MHz puts −55.000° exactly on the new 45/9 = 5° grid. Both outputs stay at 125 MHz. Full history and the per-configuration measured table in `Documents/RGMII I-O Timing Derivation.md`. **The bench half remains V-2** — a phase shift is an I/O property no simulation here can check, and 58 ps is inside what a real board's trace skew can move |
...
| R20 | WNS ≥ 0 at 125 MHz, RGMII I/O constrained | `scripts/synth_module.tcl` (WNS +1.855 ns post-synthesis, out of context, refuses on negative — re-measured after the MDIO sampling fix), `scripts/build.tcl` gates 2 and 3 | tool | **partial** — the I/O-delay half lands with the RGMII constraints in Stage 6. Stage 6 part 2 update: the constraints are wired and the RX deskew is built; gate 2 currently **refuses** (WNS −2.109 ns on the five RX input pins, hold clean at +0.094 ns) pending resolution of the structural finding in `docs/reports/stage6-part2/task-4d-report.md`. The refusing gates are the honest state and are recorded as such rather than bypassed |
...
| R24 | Bit-exact vs the golden model; no negative slack | `make regress` + the slack gates | sim + tool | **green** — 29 of 29 runs pass; WNS +1.855 ns out of context at 125 MHz (re-measured after the MDIO sampling fix) |
...
| `gem_mac` out of context (Stage 4 step 6) | **775 LUT · 793 FF · 0 BRAM · 0 DSP · WNS +1.855 ns** at 125 MHz (cell counts), re-measured after the MDIO sampling fix — the delta from the Stage-4 snapshot of 745/788/+2.262 is `gem_mdio`'s input synchroniser and end-of-period sampler |
...
| **V-2** | R14's RGMII skew cannot be simulated | The 1.2222 ns `GTX_CLK` phase shift is an I/O timing property. Simulation will pass with any phase; only static timing analysis and a scope on the bench can confirm it. | **Stage 6's static half is now done** — post-route `report_timing` against `constrs/rgmii_timing.xdc` measures +0.058 ns worst-case TX setup on the committed design (R14). What is left is the bench: bring-up step 5, an ILA or scope on `GTX_CLK`/`TXD0`, once the board is in hand. 58 ps is thin enough that the bench may well disagree, so this item no longer ends in a dead end — the next action is named: `Documents/RGMII I-O Timing Derivation.md`, section **"If 58 ps proves insufficient on the bench"**, which specifies the KSZ9031RNX `GTX_CLK` pad-skew fallback (MMD `2h`, register `8h`, bits `[9:5]`) reached over MDIO, the step-count arithmetic, and — explicitly — which of its numbers must be confirmed against the datasheet before anyone executes it. That procedure is documented, deliberately not implemented: it cannot be validated without hardware, and an unvalidated correction applied at every boot would be worse than none. |
...
| **V-24** | RGMII RX input timing cannot close on the deskewed-BUFG clocking, structurally | Measured, not suspected: the corrected constraints wired into the real build refuse at gate 2 — WNS −2.109 ns on all five RX pins, hold clean (+0.094 ns), TNS across exactly those five endpoints. The deskew MMCM collapsed the clock-network spread from 3.720 ns to 0.635 ns and fixed hold entirely; what remains is a capture interval that sits ~2 ns early and is ~0.11 ns too wide for the RGMII eye under any shift (per-port setup+hold sums uniform at −0.245 ns). A BUFH rebuild came back bit-identical, so the residue lives in the compensation model, not the routing. The design document's IDELAYE2 fallback is wrong-signed for this failure mode. | Full evidence: docs/reports/stage6-part2/task-4d-report.md, 	ask-4d2-report.md. Next actions named there: resolve the half-cycle residue question (is the tool's deskewed-clock model anti-phase to the declared waveform?) before re-deriving any margin number; the REF_JITTER1 = 0.125 second run of criterion A remains outstanding either way. impl/itstream refusing is the honest state; nothing ships past gate 2 until this closes. |
| **V-25** | An RX AXI-S frame in flight during a link event aborts without 	last | Consequence of the Stage 6 part 2 reset architecture (x_path_rst_n resets gem_rx_egress so a link drop cannot splice stale FIFO contents into a new frame as well-formed output). Owner decision (a): accept the mid-frame 	valid drop for v1, documented in B.4a's amendment. | Demonstrated behaviour must be pinned by a testbench scenario (criterion D8) once the timing question V-24 unblocks full-scenario work; before then it stays documented rather than latent. The clean in-band abort (final beat with 	last=1, tuser=1) is scheduled as its own task before anything real consumes the RX port. |
